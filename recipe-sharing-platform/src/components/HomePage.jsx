...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch("/data.json") // Assuming Vite serves from public/
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {recipes.map((recipe) => (
          <div key={recipe.id} className="bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow">
            ...
            <button className="mt-4 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition">View Recipe</button>
          </div>
        ))}
      </div>
    </div>
